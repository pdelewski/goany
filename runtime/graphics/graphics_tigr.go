...
// Default backend is tigr (no external dependencies).
// Build with SDL2: go build -tags sdl2
package graphics

import tigr "runtime/graphics/go/tigr"

// Window represents a graphics window.
type Window struct {
	handle   int64 // Pointer to Tigr
	renderer int64 // Same as handle for tigr
	width    int32
	height   int32
	running  bool
}
...
func CreateWindow(title string, width int32, height int32) Window {
	handle, renderer, ok := tigr.CreateWindow(title, width, height)
	if !ok {
