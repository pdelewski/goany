		...
		if len(node.Args) >= 1 {
			...
			if _, ok := node.Args[0].(*ast.ArrayType); ok {
				...
				if len(parts) >= 2 {
					...
					e.fs.PushTree(IRTree(CallExpression, KindExpr, Leaf(Identifier, "[]")))
				}
				return
			}
		}
...
func (e *JSEmitter) PostVisitSelectorExpr(node *ast.SelectorExpr, indent int) {
	...
	if xCode == "os" && selCode == "Args" {
		e.fs.PushTree(IRTree(SelectorExpression, KindExpr, Leaf(Identifier, "process.argv.slice(1)")))
		return
	}
...
func (e *JSEmitter) PostVisitCompositeLit(node *ast.CompositeLit, indent int) {
	...
	if litType == nil {
		...
		e.fs.PushTree(IRTree(CompositeLitExpression, KindExpr, Leaf(Identifier, "["+eltsStr+"]")))
		return
	}
...
func (e *JSEmitter) PostVisitFuncDeclSignatureTypeParamsList(node *ast.Field, index int, indent int) {
	...
	for _, t := range tokens {
		if t.Kind == TagIdent {
			e.fs.Push(t.Serialize(), TagIdent, nil)
		}
	}
}
...
func (e *JSEmitter) PostVisitFuncDeclSignature(node *ast.FuncDecl, indent int) {
	...
	if e.inNamespace {
		e.fs.PushTree(IRTree(FuncDeclaration, KindDecl,
			Leaf(NewLine, "\n"),
...
func (e *JSEmitter) PostVisitExprStmt(node *ast.ExprStmt, indent int) {
	...
	ind := jsIndent(indent / 2)
	e.fs.PushTree(IRTree(ExprStatement, KindStmt, Leaf(Identifier, ind+code+";\n")))
}
...
func (e *JSEmitter) PreVisitBranchStmt(node *ast.BranchStmt, indent int) {
	...
	switch node.Tok {
	case token.BREAK:
		e.fs.PushTree(IRTree(BranchStatement, KindStmt, Leaf(Identifier, ind+"break;\n")))
	case token.CONTINUE:
		e.fs.PushTree(IRTree(BranchStatement, KindStmt, Leaf(Identifier, ind+"continue;\n")))
	}
}
