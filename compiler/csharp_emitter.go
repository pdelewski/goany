...
func (e *CSharpEmitter) PreVisitPackage(pkg *packages.Package, indent int) {
	...
	e.fs.PushTree(TokenTree(CSharpKeyword, TagStmt,
		Leaf(CSharpKeyword, "public static class "),
			...
			if _, ok := selExpr.X.(*ast.IndexExpr); ok {
				...
				if elemType != nil {
					if _, isStruct := elemType.Underlying().(*types.Struct); isStruct {
						...
						e.nestedMapCounter++
						e.fs.PushTree(TokenTree(CSharpKeyword, TagExpr,
							Leaf(WhiteSpace, ind),
							Leaf(CSharpKeyword, "var "),
...
func (e *CSharpEmitter) PostVisitGenDeclConstName(node *ast.Ident, indent int) {
	...
	e.fs.PushTree(TokenTree(CSharpKeyword, TagExpr,
		Leaf(CSharpKeyword, "public const "),
