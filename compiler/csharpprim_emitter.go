...
// CSharpPrimEmitter implements the Emitter interface using a shift/reduce architecture
// for C# code generation. Mirrors the JSEmitter pattern from js_emitter.go.
type CSharpPrimEmitter struct {
	fs              *FragmentStack
	Output          string
	OutputDir       string
	OutputName      string
...
func (e *CSharpPrimEmitter) SetFile(file *os.File) { e.file = file }
func (e *CSharpPrimEmitter) GetFile() *os.File     { return e.file }
...
func (e *CSharpPrimEmitter) qualifiedCsTypeName(t types.Type) string {
	if t == nil {
		return "object"
	}
...
// convertGoTypeToCSharpPrim converts a Go type string to C# syntax.
func convertGoTypeToCSharpPrim(goType string) string {
	result := goType

	if strings.HasPrefix(result, "[]") {
		elementType := result[2:]
		elementType = convertGoTypeToCSharpPrim(elementType)
	...
	if strings.HasPrefix(result, "map[") {
		...
		if bracketEnd > 4 {
			...
			keyType = convertGoTypeToCSharpPrim(keyType)
			valueType = convertGoTypeToCSharpPrim(valueType)
...
func (e *CSharpPrimEmitter) analyzeLhsIndexChainCs(expr ast.Expr) (ops []csMixedOp, hasIntermediateMap bool) {
	var chain []ast.Expr
	current := expr
	for {
...
func (e *CSharpPrimEmitter) isMapTypeExpr(expr ast.Expr) bool {
...
func (e *CSharpPrimEmitter) getExprGoType(expr ast.Expr) types.Type {
	if e.pkg == nil || e.pkg.TypesInfo == nil {
		return nil
	}
	tv := e.pkg.TypesInfo.Types[expr]
...
func (e *CSharpPrimEmitter) getMapKeyTypeConst(mapType *ast.MapType) int {
	if e.pkg != nil && e.pkg.TypesInfo != nil {
...
func (e *CSharpPrimEmitter) PreVisitProgram(indent int) {
	var err error
...
`)
	...
	// Runtime packages are copied to separate .csprim.cs files by CopyRuntimePackages() in PostVisitProgram
}


func (e *CSharpPrimEmitter) PostVisitProgram(indent int) {
...
func (e *CSharpPrimEmitter) replaceStructKeyFunctions() {
...
func (e *CSharpPrimEmitter) PreVisitPackage(pkg *packages.Package, indent int) {
	e.pkg = pkg
	name := pkg.Name
	if name == "main" {
...
func (e *CSharpPrimEmitter) PostVisitPackage(pkg *packages.Package, indent int) {
	e.fs.PushCode("}\n")
}
...
func (e *CSharpPrimEmitter) PreVisitBasicLit(node *ast.BasicLit, indent int) {
	val := node.Value
...
func (e *CSharpPrimEmitter) PreVisitIdent(node *ast.Ident, indent int) {
	name := node.Name
	// Map Go builtins
	switch name {
	case "true", "false":
...
func (e *CSharpPrimEmitter) PostVisitBinaryExprLeft(node ast.Expr, indent int) {
	left := e.fs.ReduceToCode(string(PreVisitBinaryExprLeft))
	e.fs.PushCode(left)
}

func (e *CSharpPrimEmitter) PostVisitBinaryExprRight(node ast.Expr, indent int) {
	right := e.fs.ReduceToCode(string(PreVisitBinaryExprRight))
	e.fs.PushCode(right)
}

func (e *CSharpPrimEmitter) PostVisitBinaryExpr(node *ast.BinaryExpr, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitCallExprFun(node ast.Expr, indent int) {
	funCode := e.fs.ReduceToCode(string(PreVisitCallExprFun))
	e.fs.PushCode(funCode)
}

func (e *CSharpPrimEmitter) PostVisitCallExprArg(node ast.Expr, index int, indent int) {
	argCode := e.fs.ReduceToCode(string(PreVisitCallExprArg))
	e.fs.PushCode(argCode)
}

func (e *CSharpPrimEmitter) PostVisitCallExprArgs(node []ast.Expr, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitCallExpr(node *ast.CallExpr, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitSelectorExprX(node ast.Expr, indent int) {
	xCode := e.fs.ReduceToCode(string(PreVisitSelectorExprX))
	e.fs.PushCode(xCode)
}

func (e *CSharpPrimEmitter) PostVisitSelectorExprSel(node *ast.Ident, indent int) {
	e.fs.Reduce(string(PreVisitSelectorExprSel))
	e.fs.PushCode(node.Name)
}

func (e *CSharpPrimEmitter) PostVisitSelectorExpr(node *ast.SelectorExpr, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitIndexExprX(node *ast.IndexExpr, indent int) {
	xCode := e.fs.ReduceToCode(string(PreVisitIndexExprX))
	e.fs.PushCode(xCode)
	e.lastIndexXCode = xCode
}

func (e *CSharpPrimEmitter) PostVisitIndexExprIndex(node *ast.IndexExpr, indent int) {
	idxCode := e.fs.ReduceToCode(string(PreVisitIndexExprIndex))
	e.fs.PushCode(idxCode)
	e.lastIndexKeyCode = idxCode
}

func (e *CSharpPrimEmitter) PostVisitIndexExpr(node *ast.IndexExpr, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitUnaryExpr(node *ast.UnaryExpr, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitParenExpr(node *ast.ParenExpr, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitCompositeLitType(node ast.Expr, indent int) {
	e.fs.Reduce(string(PreVisitCompositeLitType))
}

func (e *CSharpPrimEmitter) PostVisitCompositeLitElt(node ast.Expr, index int, indent int) {
	eltCode := e.fs.ReduceToCode(string(PreVisitCompositeLitElt))
	e.fs.PushCode(eltCode)
}

func (e *CSharpPrimEmitter) PostVisitCompositeLitElts(node []ast.Expr, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitCompositeLit(node *ast.CompositeLit, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitKeyValueExprKey(node ast.Expr, indent int) {
	keyCode := e.fs.ReduceToCode(string(PreVisitKeyValueExprKey))
	e.fs.PushCode(keyCode)
}

func (e *CSharpPrimEmitter) PostVisitKeyValueExprValue(node ast.Expr, indent int) {
	valCode := e.fs.ReduceToCode(string(PreVisitKeyValueExprValue))
	e.fs.PushCode(valCode)
}

func (e *CSharpPrimEmitter) PostVisitKeyValueExpr(node *ast.KeyValueExpr, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitSliceExprX(node ast.Expr, indent int) {
	xCode := e.fs.ReduceToCode(string(PreVisitSliceExprX))
	e.fs.PushCode(xCode)
}

func (e *CSharpPrimEmitter) PostVisitSliceExprXBegin(node ast.Expr, indent int) {
	e.fs.Reduce(string(PreVisitSliceExprXBegin))
}

func (e *CSharpPrimEmitter) PostVisitSliceExprLow(node ast.Expr, indent int) {
	lowCode := e.fs.ReduceToCode(string(PreVisitSliceExprLow))
	e.fs.PushCode(lowCode)
}

func (e *CSharpPrimEmitter) PostVisitSliceExprXEnd(node ast.Expr, indent int) {
	e.fs.Reduce(string(PreVisitSliceExprXEnd))
}

func (e *CSharpPrimEmitter) PostVisitSliceExprHigh(node ast.Expr, indent int) {
	highCode := e.fs.ReduceToCode(string(PreVisitSliceExprHigh))
	e.fs.PushCode(highCode)
}

func (e *CSharpPrimEmitter) PostVisitSliceExpr(node *ast.SliceExpr, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitArrayType(node ast.ArrayType, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitMapKeyType(node ast.Expr, indent int) {
	e.fs.Reduce(string(PreVisitMapKeyType))
}

func (e *CSharpPrimEmitter) PostVisitMapValueType(node ast.Expr, indent int) {
	e.fs.Reduce(string(PreVisitMapValueType))
}

func (e *CSharpPrimEmitter) PostVisitMapType(node *ast.MapType, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitFuncTypeResult(node *ast.Field, index int, indent int) {
	resultCode := e.fs.ReduceToCode(string(PreVisitFuncTypeResult))
	e.fs.PushCode(resultCode)
}

func (e *CSharpPrimEmitter) PostVisitFuncTypeResults(node *ast.FieldList, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitFuncTypeParam(node *ast.Field, index int, indent int) {
	paramCode := e.fs.ReduceToCode(string(PreVisitFuncTypeParam))
	e.fs.PushCode(paramCode)
}

func (e *CSharpPrimEmitter) PostVisitFuncTypeParams(node *ast.FieldList, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitFuncType(node *ast.FuncType, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitFuncLitTypeParam(node *ast.Field, index int, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitFuncLitTypeParams(node *ast.FieldList, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitFuncLitTypeResults(node *ast.FieldList, indent int) {
	e.fs.Reduce(string(PreVisitFuncLitTypeResults))
}

func (e *CSharpPrimEmitter) PostVisitFuncLitBody(node *ast.BlockStmt, indent int) {
	bodyCode := e.fs.ReduceToCode(string(PreVisitFuncLitBody))
	e.fs.PushCode(bodyCode)
}

func (e *CSharpPrimEmitter) PostVisitFuncLit(node *ast.FuncLit, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitTypeAssertExprType(node ast.Expr, indent int) {
	typeCode := e.fs.ReduceToCode(string(PreVisitTypeAssertExprType))
	e.fs.PushCode(typeCode)
}

func (e *CSharpPrimEmitter) PostVisitTypeAssertExprX(node ast.Expr, indent int) {
	xCode := e.fs.ReduceToCode(string(PreVisitTypeAssertExprX))
	e.fs.PushCode(xCode)
}

func (e *CSharpPrimEmitter) PostVisitTypeAssertExpr(node *ast.TypeAssertExpr, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitStarExpr(node *ast.StarExpr, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitInterfaceType(node *ast.InterfaceType, indent int) {
...
func (e *CSharpPrimEmitter) PreVisitFuncDeclSignatureTypeResults(node *ast.FuncDecl, indent int) {
	e.numFuncResults = 0
...
func (e *CSharpPrimEmitter) PostVisitFuncDeclSignatureTypeResultsList(node *ast.Field, index int, indent int) {
	typeCode := e.fs.ReduceToCode(string(PreVisitFuncDeclSignatureTypeResultsList))
	e.fs.PushCode(typeCode)
}

func (e *CSharpPrimEmitter) PostVisitFuncDeclSignatureTypeResults(node *ast.FuncDecl, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitFuncDeclName(node *ast.Ident, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitFuncDeclSignatureTypeParamsListType(node ast.Expr, argName *ast.Ident, index int, indent int) {
	typeCode := e.fs.ReduceToCode(string(PreVisitFuncDeclSignatureTypeParamsListType))
	e.fs.PushCode(typeCode)
}

func (e *CSharpPrimEmitter) PostVisitFuncDeclSignatureTypeParamsArgName(node *ast.Ident, index int, indent int) {
	e.fs.Reduce(string(PreVisitFuncDeclSignatureTypeParamsArgName))
	e.fs.Push(node.Name, TagIdent, nil)
}

func (e *CSharpPrimEmitter) PostVisitFuncDeclSignatureTypeParamsList(node *ast.Field, index int, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitFuncDeclSignatureTypeParams(node *ast.FuncDecl, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitFuncDeclSignature(node *ast.FuncDecl, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitFuncDeclBody(node *ast.BlockStmt, indent int) {
	bodyCode := e.fs.ReduceToCode(string(PreVisitFuncDeclBody))
	e.fs.PushCode(bodyCode)
}

func (e *CSharpPrimEmitter) PostVisitFuncDecl(node *ast.FuncDecl, indent int) {
...
func (e *CSharpPrimEmitter) PreVisitFuncDeclSignatures(indent int) {
	e.forwardDecl = true
}

func (e *CSharpPrimEmitter) PostVisitFuncDeclSignatures(indent int) {
...
func (e *CSharpPrimEmitter) PreVisitBlockStmt(node *ast.BlockStmt, indent int) {
	e.indent = indent
}

func (e *CSharpPrimEmitter) PostVisitBlockStmtList(node ast.Stmt, index int, indent int) {
	itemCode := e.fs.ReduceToCode(string(PreVisitBlockStmtList))
	e.fs.PushCode(itemCode)
}

func (e *CSharpPrimEmitter) PostVisitBlockStmt(node *ast.BlockStmt, indent int) {
...
func (e *CSharpPrimEmitter) PreVisitAssignStmt(node *ast.AssignStmt, indent int) {
	e.indent = indent
	e.mapAssignVar = ""
	e.mapAssignKey = ""
}

func (e *CSharpPrimEmitter) PostVisitAssignStmtLhsExpr(node ast.Expr, index int, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitAssignStmtLhs(node *ast.AssignStmt, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitAssignStmtRhsExpr(node ast.Expr, index int, indent int) {
	rhsCode := e.fs.ReduceToCode(string(PreVisitAssignStmtRhsExpr))
	e.fs.PushCode(rhsCode)
}

func (e *CSharpPrimEmitter) PostVisitAssignStmtRhs(node *ast.AssignStmt, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitAssignStmt(node *ast.AssignStmt, indent int) {
...
func (e *CSharpPrimEmitter) PreVisitDeclStmt(node *ast.DeclStmt, indent int) {
	e.indent = indent
}

func (e *CSharpPrimEmitter) PostVisitDeclStmtValueSpecType(node *ast.ValueSpec, index int, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitDeclStmtValueSpecNames(node *ast.Ident, index int, indent int) {
	e.fs.Reduce(string(PreVisitDeclStmtValueSpecNames))
	e.fs.Push(node.Name, TagIdent, nil)
}

func (e *CSharpPrimEmitter) PostVisitDeclStmtValueSpecValue(node ast.Expr, index int, indent int) {
	valCode := e.fs.ReduceToCode(string(PreVisitDeclStmtValueSpecValue))
	e.fs.Push(valCode, TagExpr, nil)
}

func (e *CSharpPrimEmitter) PostVisitDeclStmt(node *ast.DeclStmt, indent int) {
...
func (e *CSharpPrimEmitter) PreVisitReturnStmt(node *ast.ReturnStmt, indent int) {
	e.indent = indent
}

func (e *CSharpPrimEmitter) PostVisitReturnStmtResult(node ast.Expr, index int, indent int) {
	resultCode := e.fs.ReduceToCode(string(PreVisitReturnStmtResult))
	e.fs.PushCode(resultCode)
}

func (e *CSharpPrimEmitter) PostVisitReturnStmt(node *ast.ReturnStmt, indent int) {
...
func (e *CSharpPrimEmitter) PreVisitExprStmt(node *ast.ExprStmt, indent int) {
	e.indent = indent
}

func (e *CSharpPrimEmitter) PostVisitExprStmtX(node ast.Expr, indent int) {
	xCode := e.fs.ReduceToCode(string(PreVisitExprStmtX))
	e.fs.PushCode(xCode)
}

func (e *CSharpPrimEmitter) PostVisitExprStmt(node *ast.ExprStmt, indent int) {
...
func (e *CSharpPrimEmitter) PreVisitIfStmt(node *ast.IfStmt, indent int) {
	e.indent = indent
...
func (e *CSharpPrimEmitter) PostVisitIfStmtInit(node ast.Stmt, indent int) {
	e.ifInitStack[len(e.ifInitStack)-1] = e.fs.ReduceToCode(string(PreVisitIfStmtInit))
}

func (e *CSharpPrimEmitter) PostVisitIfStmtCond(node *ast.IfStmt, indent int) {
	e.ifCondStack[len(e.ifCondStack)-1] = e.fs.ReduceToCode(string(PreVisitIfStmtCond))
}

func (e *CSharpPrimEmitter) PostVisitIfStmtBody(node *ast.IfStmt, indent int) {
	e.ifBodyStack[len(e.ifBodyStack)-1] = e.fs.ReduceToCode(string(PreVisitIfStmtBody))
}

func (e *CSharpPrimEmitter) PostVisitIfStmtElse(node *ast.IfStmt, indent int) {
	e.ifElseStack[len(e.ifElseStack)-1] = e.fs.ReduceToCode(string(PreVisitIfStmtElse))
}

func (e *CSharpPrimEmitter) PostVisitIfStmt(node *ast.IfStmt, indent int) {
...
func (e *CSharpPrimEmitter) PreVisitForStmt(node *ast.ForStmt, indent int) {
	e.indent = indent
...
func (e *CSharpPrimEmitter) PostVisitForStmtInit(node ast.Stmt, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitForStmtCond(node ast.Expr, indent int) {
	e.forCondStack[len(e.forCondStack)-1] = e.fs.ReduceToCode(string(PreVisitForStmtCond))
}

func (e *CSharpPrimEmitter) PostVisitForStmtPost(node ast.Stmt, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitForStmt(node *ast.ForStmt, indent int) {
...
func (e *CSharpPrimEmitter) PreVisitRangeStmt(node *ast.RangeStmt, indent int) {
	e.indent = indent
}

func (e *CSharpPrimEmitter) PostVisitRangeStmtKey(node ast.Expr, indent int) {
	keyCode := e.fs.ReduceToCode(string(PreVisitRangeStmtKey))
	e.fs.Push(keyCode, TagIdent, nil)
}

func (e *CSharpPrimEmitter) PostVisitRangeStmtValue(node ast.Expr, indent int) {
	valCode := e.fs.ReduceToCode(string(PreVisitRangeStmtValue))
	e.fs.Push(valCode, TagIdent, nil)
}

func (e *CSharpPrimEmitter) PostVisitRangeStmtX(node ast.Expr, indent int) {
	xCode := e.fs.ReduceToCode(string(PreVisitRangeStmtX))
	e.fs.PushCode(xCode)
}

func (e *CSharpPrimEmitter) PostVisitRangeStmt(node *ast.RangeStmt, indent int) {
...
func (e *CSharpPrimEmitter) PreVisitSwitchStmt(node *ast.SwitchStmt, indent int) {
	e.indent = indent
}

func (e *CSharpPrimEmitter) PostVisitSwitchStmtTag(node ast.Expr, indent int) {
	tagCode := e.fs.ReduceToCode(string(PreVisitSwitchStmtTag))
	e.fs.PushCode(tagCode)
}

func (e *CSharpPrimEmitter) PostVisitSwitchStmt(node *ast.SwitchStmt, indent int) {
...
func (e *CSharpPrimEmitter) PreVisitCaseClause(node *ast.CaseClause, indent int) {
	e.indent = indent
}

func (e *CSharpPrimEmitter) PostVisitCaseClauseListExpr(node ast.Expr, index int, indent int) {
	exprCode := e.fs.ReduceToCode(string(PreVisitCaseClauseListExpr))
	e.fs.PushCode(exprCode)
}

func (e *CSharpPrimEmitter) PostVisitCaseClauseList(node []ast.Expr, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitCaseClause(node *ast.CaseClause, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitIncDecStmt(node *ast.IncDecStmt, indent int) {
...
func (e *CSharpPrimEmitter) PreVisitBranchStmt(node *ast.BranchStmt, indent int) {
	ind := csIndent(indent / 2)
	switch node.Tok {
	case token.BREAK:
...
func (e *CSharpPrimEmitter) PostVisitGenStructFieldType(node ast.Expr, indent int) {
	typeCode := e.fs.ReduceToCode(string(PreVisitGenStructFieldType))
	e.fs.PushCode(typeCode)
}

func (e *CSharpPrimEmitter) PostVisitGenStructFieldName(node *ast.Ident, indent int) {
	e.fs.Reduce(string(PreVisitGenStructFieldName))
	e.fs.Push(node.Name, TagIdent, nil)
}

func (e *CSharpPrimEmitter) PostVisitGenStructInfo(node GenTypeInfo, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitGenStructInfos(node []GenTypeInfo, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitGenDeclConstName(node *ast.Ident, indent int) {
...
func (e *CSharpPrimEmitter) PostVisitGenDeclConst(node *ast.GenDecl, indent int) {
...
func (e *CSharpPrimEmitter) PreVisitTypeAliasName(node *ast.Ident, indent int) {
	// Store the alias name so PostVisitTypeAliasType can use it
	e.currentAliasName = node.Name
}

func (e *CSharpPrimEmitter) PostVisitTypeAliasType(node ast.Expr, indent int) {
	...
	if e.currentAliasName != "" {
		if e.pkg != nil && e.pkg.TypesInfo != nil {
			if tv, ok := e.pkg.TypesInfo.Types[node]; ok && tv.Type != nil {
				...
				underlyingType = convertGoTypeToCSharpPrim(underlyingType)
...
// ============================================================
// GenerateCsproj generates the .csproj file for the csprim backend.
// ============================================================

func (e *CSharpPrimEmitter) GenerateCsproj() error {
	if e.LinkRuntime == "" {
		return nil
	}

	csprojPath := filepath.Join(e.OutputDir, e.OutputName+".csprim.csproj")
...
`
	...
	DebugLogPrintf("Generated .csprim.csproj at %s (graphics: %s)", csprojPath, graphicsBackend)
	return nil
}

// CopyRuntimePackages copies runtime .cs files for all detected runtime packages.
func (e *CSharpPrimEmitter) CopyRuntimePackages() error {
	if e.LinkRuntime == "" {
		return nil
	}
	for name, variant := range e.RuntimePackages {
		...
		if err != nil {
			DebugLogPrintf("Skipping CsPrim runtime for %s: %v", name, err)
			continue
		}

		dstFileName := capName + "Runtime.csprim.cs"
...
func (e *CSharpPrimEmitter) csDefaultForTypeStr(typeStr string) string {
	switch typeStr {
...
func (e *CSharpPrimEmitter) csDefaultForASTType(expr ast.Expr) string {
	switch t := expr.(type) {
	case *ast.Ident:
		switch t.Name {
...
// analyzeLhsIndexChainCsPrim walks an IndexExpr chain for mixed nested assignments.
func (e *CSharpPrimEmitter) analyzeLhsIndexChainCsPrim(expr ast.Expr) (ops []csMixedIndexOp, hasIntermediateMap bool) {
	var chain []ast.Expr
	current := expr
	for {
		indexExpr, ok := current.(*ast.IndexExpr)
